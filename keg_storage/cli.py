...
def add_cli_to_app(app, cli_group_name):
    @app.cli.group(cli_group_name, chain=True,
                   help='Commands for working with remotely stored files.')
    @click.option('--location')
    @with_appcontext
    @click.pass_context
    def storage(ctx, location):
        location = location or app.config.get('KEG_STORAGE_DEFAULT_LOCATION')
        if not location:
            click.echo('No location given and no default was configured.')
            ctx.abort()
        try:
            ctx.obj.data['storage'] = app.storage.get_interface(location)
        except KeyError:
